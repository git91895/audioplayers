...
class AudioplayersPlugin : MethodCallHandler, FlutterPlugin {
    ...
    private fun onAttachedToEngine(applicationContext: Context, messenger: BinaryMessenger) {
        ...
        channel = MethodChannel(messenger, "xyz.luan/audioplayers")
        seekFinish = false
    ...
    private fun handleMethodCall(call: MethodCall, response: MethodChannel.Result) {
        ...
        when (call.method) {
            ...
            "setPlaybackRate" -> {
                val rate =
                    call.argument<Double>("playbackRate") ?: throw error("playbackRate is required")
                player.setRate(rate)
            }
            "getDuration" -> {
            ...
            "earpieceOrSpeakersToggle" -> {
                val playingRoute =
                    call.argument<String>("playingRoute") ?: throw error("playingRoute is required")
    ...
    fun handleDuration(player: Player) {
        channel.invokeMethod(
            "audio.onDuration",
            buildArguments(player.playerId, player.getDuration() ?: 0)
        )
    }
                ...
                try {
                    ...
                    if (audioplayersPlugin.seekFinish) {
                        channel.invokeMethod(
                            ...
                            buildArguments(player.playerId, true)
                        )
    ...
    companion object {
         ...
         * Plugin registration.
         */
        public fun registerWith(registrar: Registrar) {
            val instance = AudioplayersPlugin()
